    ...
    (with-temp-buffer
      (with-org-link-config
       ...
       (with-temp-buffer
         ...
         ;; (setq buffer-read-only t)
         (read-only-mode 1)
         ;; (print (list "sss1" (buffer-substring-no-properties (line-beginning-position) (line-end-position))))
         (org-open-at-point-global)
         ;; (org-links-org-open-at-point-global)
         ;; (print (line-number-at-pos))
         ;; (re-search-backward "^\\*+ headline" nil t)
         ;; (print (list "sss2" (buffer-substring-no-properties (line-beginning-position) (line-end-position))))))))
         ;; (print (list "sss2" (buffer-substring-no-properties (point-min) (point-max))))
...
(ert-deftest org-links-tests-jump-num-line-test2 ()
  (print "Test: org-links-jump-num-line-test")
  (let ((kill-buffer-query-functions))
    (with-temp-buffer
      (with-org-link-config
